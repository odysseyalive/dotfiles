Describe 'setup/common.sh'
  ...
  Describe 'get_shell_rc()'
    It 'returns ~/.zshrc when zsh is available and not in headless mode'
      unset YADR_SHELL_RC
      YADR_DIR="$(mktemp -d)"
      command() {
        if [ "$1" = "-v" ] && [ "$2" = "zsh" ]; then
    ...
    It 'returns ~/.bashrc when a .headless marker file is present'
      unset YADR_SHELL_RC
      YADR_DIR="$(mktemp -d)"
      touch "$YADR_DIR/.headless"

      When call get_shell_rc
      The output should equal "$HOME/.bashrc"
    End

    It 'returns ~/.bashrc when zsh is not on PATH'
      unset YADR_SHELL_RC
      YADR_DIR="$(mktemp -d)"
      command() {
        if [ "$1" = "-v" ] && [ "$2" = "zsh" ]; then
    ...
    It 'honors YADR_SHELL_RC override'
      YADR_SHELL_RC="/tmp/custom-rc"

      When call get_shell_rc
      The output should equal "/tmp/custom-rc"
    End
  End
